...
"#;
    ...
    pub fn new(
        ...
        let scale_loc = unsafe {
            ...
            let bytes =
                std::slice::from_raw_parts(verts.as_ptr().cast::<u8>(), verts.len() * 4);
    ...
    pub fn render(&mut self, idx: usize, frame: &Frame, mode: ScaleMode) -> anyhow::Result<()> {
        let s = self.surfaces[idx].as_mut().context("live render surface")?;
        ...
        if s.applied_video_dims != Some(video_dims) {
            let scale = compute_scale(s.dims, video_dims, mode);
            unsafe {
                self.gl.uniform_2_f32(Some(&self.scale_loc), scale.0, scale.1);
            }
            s.applied_video_dims = Some(video_dims);
        }

        let texture = glow::NativeTexture(
            NonZeroU32::new(frame.texture_id).context("zero GL texture id")?,
        );
        unsafe {
        ...
        s.surface.swap_buffers(&self.context).context("swap_buffers")?;
        Ok(())
    }
...
fn compute_scale(screen: (u32, u32), video: (u32, u32), mode: ScaleMode) -> (f32, f32) {
